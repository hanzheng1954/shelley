  ...
  <div class="tool" :data-testid="isComplete ? 'tool-call-completed' : 'tool-call-running'">
    ...
    <div v-if="isExpanded" class="tool-details">
      ...
      <div v-if="isComplete" class="tool-section">
        <div class="tool-label">{{ t("resultLabel") }}</div>
        <div :class="`tool-code ${hasError ? 'error' : ''}`">{{ resultText || t("noOutput") }}</div>
      </div>
    </div>
  </div>
</template>
...
const emoji = computed(() =>
  props.toolName === "memory" ? "🧠" : props.toolName === "dream" ? "💭" : "📋",
);
const title = computed(() => {
  if (props.toolName === "memory")
    return action.value === "save" ? t("saveMemory") : t("searchProjectMemory");
  if (props.toolName === "dream") return t("consolidateThisTask");
  return action.value === "read" ? t("restoreCheckpoint") : t("saveCheckpoint");
});
